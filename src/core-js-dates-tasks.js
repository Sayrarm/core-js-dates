...
function formatDate(date) {
  ...
  return `${formattedMonth}/${formattedDay}/${formattedYear}, ${formattedHour}:${formattedMinute}:${formattedSecond} ${ampm}`;
}
...
function getWeekNumberByDate(date) {
  ...
  return Math.ceil(((workingDate - yearStart) / 86400000 + 1) / 7);
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const schedule = [];
  let currentDate = parseDate(period.start);
  const endDate = parseDate(period.end);

  while (currentDate <= endDate) {
    for (let i = 0; i < countWorkDays && currentDate <= endDate; ) {
      schedule.push(formatDateInner(currentDate));
      currentDate.setDate(currentDate.getDate() + 1);
      i += 1;
    }
    ...
    for (let i = 0; i < countOffDays && currentDate <= endDate; ) {
      currentDate.setDate(currentDate.getDate() + 1);
      i += 1;
    }
    currentDate = new Date(currentDate.getTime());
